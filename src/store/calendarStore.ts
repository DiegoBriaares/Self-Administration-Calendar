...
interface CalendarState {
    ...
    addEvent: (date: Date, entry: { title: string; time?: string; startTime?: string; link?: string; note?: string; priority?: number | string | null; unlockDate?: string }) => Promise<void>;
    ...
    return {
        ...
        addEvent: async (date, entry) => {
            ...
            const newEvent: CalendarEvent = {
                ...
                note: entry.note?.trim() ? entry.note.trim() : null,
                link: entry.link?.trim() ? entry.link.trim() : null,
                unlockDate: entry.unlockDate || null
            };
            try {
