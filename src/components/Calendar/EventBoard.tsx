...
export const EventBoard: React.FC<EventBoardProps> = ({ selectedDate }) => {
    ...
    const [draft, setDraft] = useState({ title: '', time: '', link: '', note: '', unlockDate: '' });
            ...
            {dayEvents.length === 0 ? (
                ...
                <div className="flex flex-col gap-3">
                    {dayEvents.map((event: CalendarEvent) => {
                        const content = event.unlockDate && new Date(event.unlockDate) > new Date() ? (
                            // LOCKED STATE
                            <div className="p-2 flex flex-col items-center justify-center text-stone-400 gap-2 min-h-[60px]">
                                ...
                                {/* Author (the current user) can still see they locked it, but maybe we hide content too?
                                    For now, let's just blur it or show a peek if it's THEIR event. 
                                    Actually, "Time Capsule" implies even YOU shouldn't open it? 
                                    Let's allow editing (to remove lock) but visually it looks locked. 
                                */}
                                {/* If it's your own event, we show a button to Peek/Edit */}
                                <button
                                    onClick={() => {
                                    ...
                                    <div className="flex items-center gap-3 text-sm text-stone-500">
                                        {event.startTime && (
                                            <span className="pill-soft flex items-center gap-1">
                                                <Clock className="w-3 h-3 text-orange-500" /> {event.startTime}
                                            </span>
                                        )}
                                        {viewMode !== 'friend' && (
                                            <>
                    ...
                    <div className="flex gap-3 justify-end mt-4">
                        {editingId && (
                            <button
                                onClick={() => {
                                    setEditingId(null);
                                    setDraft({ title: '', time: '', link: '', note: '', unlockDate: '' });
                                }}
